namespace eEvolution.Sign.Cli
{
  ...
  internal sealed class Pkcs11TokenLocalCommand : Command
  {
    #region Fields
    ...
    // ".\eEvolution.Sign.exe" code pkcs11-token-local *.dll -kvc 9505299957EED70137B0CD033DDB3F2AA027A6DB -d "eEvolution GmbH & Co. KG" -u http://eEvolution.de -kvu https://localhost:7036 -kvt spilker -kvi eEvolution.Sign -kvs Ts52Xavc8hM -o signed\"
    ...
    internal Pkcs11TokenLocalCommand(CodeCommand codeCommand, IServiceProviderFactory serviceProviderFactory)
        : base("pkcs11-token-local", "Verwenden Sie Pkcs11-Token-Local")
    {
      ...
      services.AddSingleton<IKeyVaultService>(new Pkcs11KeyVaultServiceWrapper(useLocalClient: true));
