...
namespace nGroup.Sign.Pkcs11.WebApi
{
  ...
  public partial class RemoteSignClient : IPkcs11TokenAccessApi, IWebApiClient<RemoteSignClient>
  {
    #region Methods
    ...
    Task<byte[]> IPkcs11TokenAccessApi.GetCertificateAsync(string credential, string certificateName)
    {
    ...
    Task<byte[]> IPkcs11TokenAccessApi.RsaSignHashAsync(string credential, string certificateName, byte[] hash, HashAlgorithmName hashAlgorithmName, RSASignaturePadding signaturePadding)
    {
      return this.RsaSignHashAsync(credential, certificateName, hash, hashAlgorithmName.Name, signaturePadding.Mode.ToString());
    }

    Task<bool> IPkcs11TokenAccessApi.RsaVerifyHashAsync(string credential, string certificateName, byte[] hash, byte[] signature, HashAlgorithmName hashAlgorithmName, RSASignaturePadding signaturePadding)
    {
